...
class HttpClient:
    def __init__(self, policy, evidence_writer, timeout: int):
        self.policy = policy
    ...
    def get(self, url: str, module: str = "unknown", headers: Optional[Dict[str,str]] = None) -> Dict[str, Any]:
        self.policy.assert_allowed(url, "GET")
        req = {"method":"GET", "url":url, "headers":headers or {}}
        try:
            r = requests.get(url, headers=headers, timeout=self.timeout)
            resp = {"status": r.status_code, "headers": dict(r.headers), "body": r.text[:200000]}
        except Exception as e:
            resp = {"status": 0, "error": str(e), "headers": {}, "body": ""}
